        ...
        return $(this).each(function() {
            ...
            if( $(this).is('input') && $(this).attr('type') == "text" ) {
                ...
                if(dd_input.val()) {
                    ...
                    var txt = dd_input.val();
                    //console.log(settings.format)
                    //hard code for format 01-Jan-2005, fix date format we set in , Eranga start
                    txt=dateDroperFormatFix(txt);
                    txt=getDateString('MM/DD/YYYY', txt, false, ''); 
                    //hard code for format 01-Jan-2005, fix date format we set in , Eranga end 
                    
                    var number_regex = txt.match(/(?:\d{4}|\d{1,2})/g),
                    ...
                    if( number_regex) {
                        ...
                        if(tempM<10) {
                            if(tempM.length==2) tempM = tempM.substr(1);
                        }
                        if(tempD<10) {
                            if(tempD.length==2) tempD = tempD.substr(1);
                        }
                ...
                setDateAnimate 	= function() {
                    dd_m.find('.dd_sl_').animate({
                        scrollLeft:dd_m.find('li.dd_sltd_').index()*range
                    },1200,'swing');
                    setTimeout(function(){
                        dd_d.find('.dd_sl_').animate({
                            scrollLeft:dd_d.find('li.dd_sltd_').index()*range
                        },1200,'swing');
                        setTimeout(function(){
                            dd_y.find('.dd_sl_').animate({
                                scrollLeft:dd_y.find('li.dd_sltd_').index()*range
                            },1200,'swing');
                        },200);
                    },400);
                },
                ...
                dropperSubmit = function(str) {
                    //hard code for format 01-Jan-2005, fix date format we set in , Eranga start 
                    str=getDateString('DD-Mon-YYYY', str, false, '');                      
                    //hard code for format 01-Jan-2005, fix date format we set in , Eranga end 
                ...
                if(settings.years_multiple) {
                    ...
                    dd_y_r.find('li').on('click',function(){
                        ...
                        dd_y.find('.dd_sl_').stop().animate({
                            scrollLeft:(dd_y.find('li[value='+x+']').index())*range
                        },600,'swing');
                ...
                dd_submit.click(function(){
                    ...
                    if(settings.lock) {
                        ...
                        if(settings.lock=='from') {
                            if(d2 < d1) dropperAlert(); else dropperSubmit(str);
                        }
                        else {
                            if(d2 > d1) dropperAlert(); else dropperSubmit(str);
                        }
